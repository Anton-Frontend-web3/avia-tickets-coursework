...
const daysOfWeek = [
  { id: '1', label: 'Понедельник' }, { id: '2', label: 'Вторник' },
  { id: '3', label: 'Среда' }, { id: '4', label: 'Четверг' },
  { id: '5', label: 'Пятница' }, { id: '6', label: 'Суббота' },
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 max-w-2xl mx-auto">
        <FormField control={form.control} name="flight_number" render={({ field }) => (
            <FormItem>
                ...
                <Input placeholder="SU-001" {...field} />
                <FormMessage />
            </FormItem>
        )} />

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormField control={form.control} name="departure_airport_id" render={({ field }) => (
                <FormItem>
                    <FormLabel>Аэропорт вылета</FormLabel>
                    <Select onValueChange={field.onChange} value={field.value}>
                        <FormControl>
                            <SelectTrigger><SelectValue placeholder="Выберите аэропорт" /></SelectTrigger>
                        </FormControl>
                        <SelectContent>{airports.map(airport => <SelectItem key={airport.value} value={airport.value}>{airport.label}</SelectItem>)}</SelectContent>
                    </Select>
                    <FormMessage />
                </FormItem>
            )} />
            <FormField control={form.control} name="arrival_airport_id" render={({ field }) => (
                <FormItem>
        ...
        <FormField control={form.control} name="days_of_week" render={({ field }) => (
            <FormItem>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2">
                    {daysOfWeek.map((day) => (
                        <FormField key={day.id} control={form.control} name="days_of_week" render={({ field }) => (
