    ...
    <div className="flex justify-between items-center py-3 border-b last:border-0">
      ...
      <div className="flex items-center gap-3">
        <Button 
          ...
          variant="outline" 
          size="icon" 
          className="h-8 w-8 rounded-md bg-slate-100 border-none hover:bg-slate-200 text-slate-600" 
          onClick={onDecrement}
          disabled={count <= min}
        >
        ...
        <Button 
          ...
          variant="outline" 
          size="icon" 
          className="h-8 w-8 rounded-md bg-blue-50 border-none hover:bg-blue-100 text-blue-600" 
          onClick={onIncrement}
          // Кнопка "+" блокируется, если достигнут ОБЩИЙ лимит
          // ИЛИ (для младенцев) если их количество догоняет количество взрослых
          disabled={totalCount >= MAX_PASSENGERS || (label === 'Младенцы' && count >= counts.adults)}
        >
