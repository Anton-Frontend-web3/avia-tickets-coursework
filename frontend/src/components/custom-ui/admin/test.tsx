...
export function ScheduleForm({
    ...
    const airportItems = airports.map(airport => (
        <SelectItem key={airport.value} value={airport.value}>
             <span className="block truncate max-w-[280px]">{airport.label}</span>
        </SelectItem>
    ));
    return (
        <Form {...form}>
...
<div className='grid gap-6 md:grid-cols-2'>
                ...
                <FormField
                    ...
                    render={({ field }) => {
                        // НОВАЯ ЛОГИКА: Получаем ID вылета и находим аэропорт
                        const departureId = form.watch('departure_airport_id');
                        const departureAirport = airports.find(a => a.value === departureId);
                                ...
                                >
                                    ...
                                    <SelectContent className="max-h-[300px] w-[var(--radix-select-trigger-width)]">
                                        {airports.map(airport => {
                                            // НОВАЯ ЛОГИКА: Парсим город прибытия и проверяем совпадение
                                            const arrivalCity = airport.label.split(' - ')[0].trim();
                                            const isSameCity = departureCity && arrivalCity === departureCity;

                                            return (
                                                <SelectItem
                                                    key={airport.value}
                                                    value={airport.value}
                                                    disabled={airport.value === departureId || !!isSameCity}
                                                >
                                                    <span className="block truncate max-w-[280px]">{airport.label}</span>
                                                </SelectItem>
                                            );
