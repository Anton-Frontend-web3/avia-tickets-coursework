...
export function SeatMap({ flightId, onSelectionChange, maxSeats }: SeatMapProps) {
    ...
    if (!layout) return <div className="p-4 text-center text-gray-500">Загрузка карты мест...</div>
    ...
    return (
        <div className="flex flex-col">
            ...
            <div className="flex flex-col gap-2 overflow-auto max-h-[400px] border p-4 rounded-xl bg-white shadow-sm">
                {/* ЛЕГЕНДА */}
                <div className="flex justify-center mb-6 text-xs gap-6">
                    <div className="flex items-center gap-2">
                        <div className="w-5 h-5 bg-gray-200 rounded"/> 
                        <span className="text-gray-500">Занято</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="w-5 h-5 bg-blue-50 border border-blue-200 rounded"/> 
                        <span className="text-gray-600">Свободно</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="w-5 h-5 bg-blue-600 rounded shadow-sm"/> 
                ...
                {Array.from({ length: layout.rows }).map((_, i) => {
                    ...
                    return (
                        <div key={rowNum} className="flex items-center justify-center gap-3">
                            <span className="w-6 text-xs font-medium text-gray-400 text-center">{rowNum}</span>
                                ...
                                return (
                                    <div key={seatId} className="flex">
                                        <button
                                            ...
                                            className={cn(
                                                "w-9 h-9 rounded-md text-xs font-bold transition-all duration-200 flex items-center justify-center",
                                                isTaken 
                                                    ? "bg-gray-100 text-gray-300 cursor-not-allowed" 
                                                    : isMySelection
                                                        ? "bg-blue-600 text-white shadow-md scale-105 ring-2 ring-blue-200 ring-offset-1"
                                                        : "bg-blue-50 text-blue-900 hover:bg-blue-100 hover:scale-105 border border-blue-200"
                                            )}
                                        >
                                            {letter}
                                        </button>
