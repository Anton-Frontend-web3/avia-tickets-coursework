...
export async function getFlightDetailsById(flightId: string): Promise<IFlight | null> {
    try {
        const sqlQuery = `
            SELECT
                f.flight_id, 
...
export async function getAirportsForSelect(): Promise<SelectOption[]> {
    try {
      const result = await pool.query('SELECT airport_id, airport_name, city, iata_code FROM airports ORDER BY city, airport_name');
...
export async function getPassengersByFlightId(flightId:string ):Promise<IPassengerCheck[]> {
