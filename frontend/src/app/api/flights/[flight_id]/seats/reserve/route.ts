...
export async function POST(req: Request, { params }: RouteContext) {
    ...
    if (!Array.isArray(seatNumbers)) { // Разрешаем пустой массив (для снятия выбора), но не null
    ...
    try {
        ...
        for (const seat of seatNumbers) {
            ...
            if ((booked.rowCount ?? 0) > 0) throw new Error(`Место ${seat} уже выкуплено`);
        ...
        return NextResponse.json({ success: true, expiresAt: new Date(Date.now() + 10 * 60000) });
