...
export default async function CheckInSeatSelectionPage({ params }: PageProps) {
    ...
    const allCheckedIn = session.passengers.every((p: any) => p.check_in_status === 'Checked-in');
    if (allCheckedIn) {
    ...
    if (!timeStatus.isOpen) {
        return <div className="p-10 text-center text-red-500">{timeStatus.message}</div>;
    }

    return (
        <div className="container mx-auto py-8 px-4 max-w-6xl">
            <h1 className="text-2xl font-bold mb-6">Выбор мест</h1>
