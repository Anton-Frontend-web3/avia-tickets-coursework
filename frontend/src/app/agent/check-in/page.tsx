...
export default  function ListPassengersPage() {
    ...
    const handleSearch = useCallback(async () => {
        ...
        setError(null);
        try {
            const response = await fetch(`/api/flights/${searchedFlightId}/passengers`, { cache: 'no-store' });
            if (!response.ok) {
    ...
    const renderContent = () => {
        ...
        if (passengers.length > 0) {
            return <PassengerList passengers={passengers} onStatusChange={handleSearch} />
        }else {
