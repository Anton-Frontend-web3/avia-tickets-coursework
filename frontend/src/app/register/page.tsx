...
export default function RegisterPage() {
  ...
  const handleRegister = async (data: z.infer<typeof formSchema>) => {
    try {
      ...
      if (!result) {
        toast.error('Ошибка регистрации', { description: 'Не удалось получить ответ от сервера.' });
        return; // Выходим из функции
      }

      // Теперь TypeScript уверен, что `result` - это объект
      if (result.success) {
        toast.success('Регистрация прошла успешно!', { description: 'Теперь вы можете войти.' });
        router.push('/login');
        ...
        toast.error('Критическая ошибка', { description: 'Не удалось выполнить действие.' });
    }
  };
