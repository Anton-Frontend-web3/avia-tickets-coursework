...
-- 6. ГЕНЕРАЦИЯ РАСПИСАНИЯ (Магия здесь)
-- Этот блок создаст рейсы из каждого города в каждый другой город (кроме внутригородских)
DO $$
DECLARE
    dep_r RECORD;
    arr_r RECORD;
    ...
    offset_day INT;
BEGIN
    -- Перебираем все аэропорты вылета
    FOR dep_r IN SELECT * FROM Airports LOOP
        -- Перебираем все аэропорты прилета
        FOR arr_r IN SELECT * FROM Airports LOOP
            
            -- Условие: ID разные И Города разные (чтобы не летать из Внуково в Домодедово)
            IF dep_r.airport_id != arr_r.airport_id AND dep_r.city != arr_r.city THEN
                
                -- Генерируем случайное время вылета (с 06:00 до 23:00)
                dep_time := make_time(floor(random() * 18 + 6)::int, floor(random() * 60)::int, 0);
                
                -- Генерируем длительность полета (от 2 до 5 часов)
                duration := make_interval(hours => floor(random() * 4 + 2)::int);
                
                -- Считаем время прилета
                arr_time := dep_time + duration;
                
                -- Если время прилета меньше времени вылета (переход через полночь), ставим offset = 1
                ...
                -- Берем случайный код авиакомпании для красоты номера рейса
                SELECT iata_code INTO airline_code FROM Airlines ORDER BY RANDOM() LIMIT 1;

                -- Вставляем расписание (триггер в базе сам создаст рейсы Flights на 30 дней)
                INSERT INTO Schedules (
                    ...
                    arr_time,
                    offset_day,
                    '{1,2,3,4,5,6,7}' -- Рейсы каждый день
                );
...
-- 8. Создаем одно тестовое бронирование на любой случайный рейс
